...
Касательно влияния ThreadPool.SetMinThreads и ThreadPool.SetMaxThreads:
1. Каждая операция по выделению потока, кроме того, что является ресурсоёмкой, так ещё и происходит с задержкой, т.е. если нам заранее известно о необходимости провести множество небольших операций используя множество потоков, имеет смысл увеличить минимальное количество потоков в пуле заранее, тем самым как минимум избежим временных задержек самого пула потоков.
2. Так как в нашем примере каждый поток после вывода в консоль тех. инфы "засыпает" на 10 сек, тем самым блокируя возможность пула потоков            использовать этот поток снова, приводит к следующему:
    1. Если минимум потоков в пуле будет достаточно высоким (порядка 850 в моём случае было достаточно), скорость прохода цикла будет (условно) ограничена лишь скоростью вывода на экран инфы, т.е. эффект от Thread.Sleep(10 sec) не будет видно (~850ый цикл начнётся через ~10 секунд, т.е. пройдёт 10сек сна первых потоков, появятся свободные для переиспользования потоки и пулу не придётся тратить время и ресурсы для создания новых), но потребует максимум (в сравнении с другими вариантами) памяти в один момент времени.
    2. Если минимум потоков будет небольшим, (в данном варианте максимум потоков достаточно установить в мин+50, при этом чем выше мин, тем меньше разницы с макс необходимо), пока мы не используем все свободные потоки, будет как в пункте 2.1, но как только количество свободных потоков в пуле закончится, пул начнёт выделять новые потоки до тех пор, пока система не придёт в некий "баланс" - скорость освобождения потоков для повторного использования превысит задержку выделения новых потоков, т.е. пул потоков практически не будет выделять новые, а станет переиспользовать освободившиеся. Потребление памяти в начале будет расти, но стабилизируется на моменте прихода системы в "баланс".
    3. Если установить мин и макс равными, то цикл (при небольших мин/макс и если пренебречь временем вызова методов и выводом текста в консоль) будет выполнятся "блоками" с задержкой в указанное время блокировки потока (Thread.Sleep(10 sec)), т.е. пулу потоков будет неоткуда брать новые потоки и придётся ждать освобождения использованных.
    4. При том же условии что и п.2.3, но при большем значении мин/макс, будет ближе к п.2.1, т.к. задержка вызова методов и вывода теста уже будет в сумме приближаться к Thread.Sleep(10 sec) и пока израсходуются все свободные потоки, следующий "блок" начнёт выполнятся тем раньше чем больше изначально свободных потоков в пуле.
(В принципе п.2.3 и п.2.4 можно было объединить в один)
